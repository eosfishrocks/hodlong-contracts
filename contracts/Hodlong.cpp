...
namespace bpfish {
    void hodlong::buy(name buyer, name storage_id) {
        hodlong::storage storage(_self, _self.value);
        ...
        if (foundStat) {
            ...
            for (int v1 = 0; pstats->pending_stats.size(); v1++) {
                for (int v2 = 1; pstats->pending_stats.size(); v2++) {
                    int v3 = v1 + v2;
                    if ((v1 != v3) && ((pstats->pending_stats[v1].to == pstats->pending_stats[v3].from ||
                                        pstats->pending_stats[v1].from == pstats->pending_stats[v3].to) &&
                                       (pstats->pending_stats[v1].to != pstats->pending_stats[v3].to &&
                                        pstats->pending_stats[v1].from != pstats->pending_stats[v3].to))) {
                        ...
                        if (now() - pstats->pending_stats[v1].submitted < 604800) {
                            ...
                            if (pstats->pending_stats[v1].amount > pstats->pending_stats[v3].amount) {
                                ...
                                verifiedAmountModifier =
                                        pstats->pending_stats[v1].amount - pstats->pending_stats[v3].amount;
                                ...
                                verifiedAmountModifier =
                                        pstats->pending_stats[v3].amount - pstats->pending_stats[v1].amount;
                            }
